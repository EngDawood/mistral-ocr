#!/usr/bin/env python3
"""
Advanced PDF to Markdown converter using Mistral OCR with Image Extraction and Output Validation.
...
- Output validation: Post-processing validation to detect common OCR issues
...
USAGE EXAMPLES:
    # Process single file
    python pdf_to_txt_with_validation.py document.pdf

    # Process all PDFs in directory (recursive)
    python pdf_to_txt_with_validation.py ./documents/

    # Custom tracking file
    python pdf_to_txt_with_validation.py ./pdfs --track-file my_log.csv --track-format csv
...
- Validation warnings for potential OCR issues
"""
...
def validate_output(markdown_text: str, pdf_path: Path) -> list[str]:
    """Validate OCR output and detect potential issues.

    Args:
        markdown_text: The markdown text output from OCR
    ...
    if "img-" in markdown_text and "image_base64" not in markdown_text:
        issues.append("Missing image data")

    # Check for excessive newlines (formatting quirk)
    if "\n\n\n\n" in markdown_text:
        issues.append("Formatting quirks detected")
...
def convert_pdf_to_txt(pdf_path: Path, model: str, output_path: Path = None) -> tuple[Path, int]:
    ...
    validation_issues = validate_output(markdown_content, pdf_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert PDF(s) to markdown using Mistral OCR with image extraction and output validation. Can process a single PDF file or all PDFs in a directory.",
    )
